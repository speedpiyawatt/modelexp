...
- [x] Add cutoff-safe nearby-station Wunderground context and rerun the full selection/calibration stack.
  ...
  - Calibration note 2026-04-30: `global_offsets_no_upper_tail` keeps the learned q05/q10/q25/q50 offsets but sets q75/q90/q95 offsets to `0.0`. This is the current production quantile calibration because it preserved the validation NLL gain while avoiding the upper-tail probability damage observed on tight/moderate consensus days.
  ...
  - Evaluator note 2026-04-30: `withhrrr_model.evaluate` now scores the same production stack as online inference by default: selected quantile calibration, selected distribution method, and selected ladder calibration. It writes `validation_predictions.parquet` for calibrated/scored rows and `validation_predictions_raw.parquet` for raw model rows.
...
- [x] Add validation-selected no-HRRR/with-HRRR inference guard.
  - Done 2026-04-30: added `tools/weather/dual_guard.py` and `tools/weather/evaluate_dual_guard.py`. Candidate guards include always no-HRRR, always with-HRRR, with-HRRR except `native_cold_hrrr_warm`, with-HRRR only on high/very-high disagreement, probability blend by regime, and expected-Tmax blend by regime.
  - Eval 2026-04-30: `.venv/bin/python tools/weather/evaluate_dual_guard.py` selected `with_hrrr_except_native_cold_hrrr_warm` using the production validation holdout with the retained 2026 server-dual backtest as confirmation. Artifacts: `experiments/withhrrr/data/runtime/evaluation/dual_guard/dual_guard_manifest.json`, `dual_guard_summary.csv`, and `dual_guard_slices.csv`.
  - Eval 2026-04-30: validation holdout `219` rows: always with-HRRR event NLL/observed-bin probability/MAE `1.133178/0.385604/1.265688`; selected guard `1.137793/0.385989/1.250839`. 2026 live backtest `86` rows: always with-HRRR `1.589559/0.408022/2.085680`; selected guard `1.554194/0.411389/2.024112`, top-bin accuracy unchanged at `0.500`.
  - Done 2026-04-30: `tools/weather/run_server_dual_inference.py` now reads the guard manifest when present and prints a `guarded` recommendation with the applied with-HRRR probability/expected weights.
  - Verification 2026-04-30: `.venv/bin/python -m py_compile tools/weather/dual_guard.py tools/weather/evaluate_dual_guard.py tools/weather/run_server_dual_inference.py experiments/withhrrr/withhrrr_model/source_disagreement.py experiments/withhrrr/withhrrr_model/predict.py` passed; `.venv/bin/python -m pytest experiments/withhrrr/tests/test_withhrrr_model.py` passed (`31 passed`); server py_compile passed after sync.
