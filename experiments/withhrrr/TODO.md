...
- [x] Add cutoff-safe nearby-station Wunderground context and rerun the full selection/calibration stack.
  ...
  - Eval 2026-04-30: refreshed holdout `2025-05-27..2025-12-31`, `219` rows. Event-bin NLL/Brier `1.345150/0.612207`; degree NLL/RPS `2.099877/0.010060`; q50 MAE/RMSE `1.261223/1.639528`.
  ...
  - Verification 2026-04-30 after review fixes: `.venv/bin/python -m py_compile wunderground/*.py experiments/withhrrr/withhrrr_model/*.py tools/weather/run_server_dual_inference.py` passed; `.venv/bin/python -m pytest experiments/withhrrr/tests/test_withhrrr_model.py` passed (`28 passed`); all-nearby inference smoke passed; no-nearby inference smoke failed explicitly as intended.
  - Deploy status 2026-04-30: code pushed to GitHub `origin/main` as commit `dbaff9a`. The DigitalOcean server `/root/modelexp` has not yet been pulled/synced for this commit, and ignored runtime artifacts still need server sync before production-style server dual inference is current.
