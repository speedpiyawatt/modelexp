...
DEFAULT_QUANTILE_CALIBRATION_MANIFEST_PATH = pathlib.Path("experiments/no_hrrr_model/data/runtime/evaluation/calibration_selection/rolling_origin_calibration_manifest.json")
DEFAULT_OUTPUT_DIR = pathlib.Path("experiments/no_hrrr_model/data/runtime/evaluation/ladder_calibration")
...
def ladder_records(prediction_df: pd.DataFrame) -> pd.DataFrame:
    min_temp_f, max_temp_f = ordered_ladder_bounds(prediction_df)
    records: list[dict[str, object]] = []
    for _, row in prediction_df.iterrows():
        ladder = quantiles_to_degree_ladder(prediction_quantiles(row), min_temp_f=min_temp_f, max_temp_f=max_temp_f)
...
def main() -> int:
    ...
    calibration_records = ladder_records(calibration_frame)
    test_records = ladder_records(test_frame)
